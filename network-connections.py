...
#               device names in the scans.csv file. This script only works on mac devices, due to desktop notifications. Will need to optimize this... >-(.-.)-<
#
...
for n in nodes:
    ...
    ips_record.append(ip);

while 1:
    for i in range(2, 25):
        ip = '192.168.0.{}'.format(i)
        print("checking misc ip {}".format(ip))
        ping_cmd = 'ping -c 1 -W 1 {} | grep "[0-9] packets received" | cut -f4 -d" "'.format(ip)
        ping_run = subprocess.Popen(ping_cmd, shell=True, stdout=subprocess.PIPE)
        packets = ping_run.communicate()[0][:-1].decode("utf-8")
        if packets == "1" and (ip not in ips_record):
            print("Watch out: Unkown device connected to your wifi! ")
            notification( "Watchout!", "[{}] Unkown device connected to your wifi! ".format(ip))

        #Fast checking recorded data while checking all the other possible misc devices
    for person in people:
        print("Checking "+person.name)
        ping_cmd = 'ping -c 1 -W 1 {} | grep "[0-9] packets received" | cut -f4 -d" "'.format(person.ip)
        ping_run = subprocess.Popen(ping_cmd, shell=True, stdout=subprocess.PIPE)
        packets = ping_run.communicate()[0][:-1].decode("utf-8")
